...
export function getGmailComposeUrl({ name = '', email = '', phone = '', service = '', message = '' } = {}) {
  ...
  const bodyLines = [
    ...
    ``,
    `----------------------------------------`,
    `Sent from website contact form to ${TARGET_EMAIL}`
  ];
...
export function openMailClient(params = {}) {
  const { gmailUrl, mailtoUrl } = getGmailComposeUrl(params);
  
  // Open Gmail web compose window directly in new tab
  const openedWindow = window.open(gmailUrl, '_blank', 'noopener,noreferrer');
  
  // Fallback to mailto if popup blocker stopped window.open
